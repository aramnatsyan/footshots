    ...
    public function search_following_list($data)
      {
       ...
       if($verify != 0){
            ...
            $result = $this->User_model->search_following_list($data);            /*Call Model function to get blocked user list */
        if($result){
            $message = array(
                'err_code' => 0,                                                 /*Return if successful */
                'message'  => 'following list loaded successfully.',
                'result'   => $result ,
          ...
          $message = array(
                 'err_code' => 0,                                                 /*Return if successful */
                'message'  => 'following list list loaded successfully.',
                'result'   => $result ,
                "has_more" => (count($result)==20)
                );
          return ($message); 
...
//   return $this->json_out($data, 0, $code = 'form_validation_error', $msg = 'Some Fields are required');
// }
        ...
        public function update_new_profile($data)
      {
        ...
        $this->form_validation->set_rules('user_pic', "profile pic", 'required|mimes:jpeg,png,jpg');
       if (!$this->form_validation->run()) {
 ...
 public function search_user_list_v2($data)
      {
       ...
       if($verify != 0){
            ...
            $verifyUser = $this->User_model->user_verify_token($data);
            $verifyUser = array_shift($verifyUser);
            $result = $this->User_model->search_user_list_v2($data);            /*Call Model function to get blocked user list */
        if($result){
            $message = array(
                'err_code' => 0,                                                 /*Return if successful */
                'message'  => 'Users list loaded successfully.',
   ...
   $commetnDetail = $this->Comment_model->single_comment($_POST['post_id'],$_POST['user_id']); 
    ...
    /*    }else{       
...

      {
        ...
        $result1 = $this->User_model->check_following($data);
      ...
      {
          $this->form_validation->set_rules('chat_name', "chat name", 'trim|required');
       if (!$this->form_validation->run()) {
...
//   $minimum_point_ro_redeem = 500; 
         ...
         public function suggestion_list($data)
      {
       ...
       if($verify != 0){
            ...
            $verifyUser = $this->User_model->user_verify_token($data);
            $verifyUser = array_shift($verifyUser);
         $result = $this->User_model->suggestion_list($data);            /*Call Model function to get blocked user list */
        if($result){
            $message = array(
                'err_code' => 0,                                                 /*Return if successful */
                'message'  => 'Users list loaded successfully.',
                'result'   => $result ,
                // "has_more" => (count($result)==10)
                );
          return ($message); 
        }else{                                                                /*Return if failure */
          $result = array();
