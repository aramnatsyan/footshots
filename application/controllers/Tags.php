...
class Tags extends CI_Controller { /**
	...
	{		
        ...
        $data['pageno'] = $pageno;
         $result = $this->Post_model->ajax_list_items();

         $data['tags_list'] =  $result;
 
         $data['posturl'] =  base_url().'tags/ajax_list_items';

        $this->load->view('site_tags', $data);
    }
 ...
 public function ajax_list_items()
    {
        ...
        if($this->input->is_ajax_request())
        {
            ...
            $first_btn = false;
            $last_btn = false;
            $response=$this->Post_model->ajax_list_items($search,$per_page,$start);
            $data['result']=$response;
             $data['pperpage']=$start+1;
            $views= 'tags_ajax_list_items';
