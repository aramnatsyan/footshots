...
function decodeEmoticons($src) {
  ...
  function send_email_sendgrid_api($to, $to_names, $subject, $text,$attachment = null,$filename = null) {
...
$from = [
    'email'=>'footshotsptyltd@gmail.com',
    'name'=>'Footshot'
    ];     
$reply_to = [
    'email'=>'footshotsptyltd@gmail.com',
...
$header = array(
    "authorization: Bearer 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",
...
//   echo $response;
 ...
 return true;
  ...
  public function send_email_sendgrid_api_old($to,$to_names,$subject,$text)
...
public function all_inactive_ajax_users_list($search='',$per_page=10,$start=0) 
    {
       $date = date('Y-m-d');
        ...
        if($search!='')
        {
          ...
          $where = " status='Blocked' and user_type='user'";
   $this->db->where($where);
        $this->db->order_by('ch.user_id', 'Desc');
        $this->db->limit($per_page,$start);
       $resultPosts =$this->db->get()->result();
  ...
  public function all_active_ajax_users_list($search='',$per_page=10,$start=0) 
    {
       $date = date('Y-m-d');
        ...
        if($search!='')
        {
          ...
          // $where = " DATE_FORMAT(ch.last_activity,'%Y-%m-%d') >='$old_date'  and user_type='user'";
  $where = " status='Active' and user_type='user'";
   $this->db->where($where);
        $this->db->order_by('ch.user_id', 'Desc');
        $this->db->limit($per_page,$start);
       $resultPosts =$this->db->get()->result();
...
$datas = array(                                                      /*An array of data to insert into table*/
  ...
  public function block_User($data)
  {
       ...
       $other_user_id =  $data->other_user_id; 
        ...
        if($restultsfollow)
        {
            if($restultsfollow->block_status == 'active')
            {
             	$datas = array(                                                      /*An array of data to insert into table*/
                	'block_status' => "delete"
...
//                 $this->db->update('foot_follow', $dataUsers); 
                    $this->db->select('foot_notification.notification_id');
                $this->db->from('foot_notification'); 
                $where = "user_id='$user_id' and from_id='$other_user_id' and notification_status='active'";
                $this->db->where($where);    
                $query = $this->db->get();
                $restults = $query->result();
                if($restults)
                ...
                // $where = "notification_id='$value->notification_id'";
                  $this->db->where_in('notification_id', $noti_id);
                  $this->db->update('foot_notification', $dataUsers);
                }
        $this->db->select('foot_likes.like_id');
                $this->db->from('foot_likes'); 
                $this->db->join('foot_posts',"foot_posts.post_id = foot_likes.post_id"); 

                $where = "foot_likes.user_id='$other_user_id' and foot_likes.like_status='active'";
                ...
                foreach($restults as $value)
                {
                  $like_ids[]=$value->like_id;
                  
                }
                // $where = "like_id='$value->like_id'";
                  $this->db->where_in('like_id', $like_ids);
                  $this->db->update('foot_likes', $dataUsers);
                }
        
        
                $this->db->select('foot_comments.comment_id');
                $this->db->from('foot_comments'); 
                $this->db->join('foot_posts',"foot_posts.post_id = foot_comments.post_id"); 

                $where = "foot_comments.user_id='$other_user_id' and foot_comments.comment_status='active'";
...
// $query = $this->db->query("(SELECT fullname,profilecover,profileimage,total_post,status,user_id,username FROM foot_app_users  where user_type='user' and fullname LIKE '$search_user_text%' and status='Active' and (select count(*) from foot_blocked_user where (other_user_id='".$user_id."' and foot_blocked_user.user_id=foot_app_users.user_id and block_status='active')  or (other_user_id=foot_app_users.user_id and foot_blocked_user.user_id='".$user_id."' and block_status='active')  )  ='0' ORDER BY `foot_app_users`.`fullname`  ASC ) union (SELECT fullname,profilecover,profileimage,total_post,status,user_id,username FROM foot_app_users  where   user_type='user' and fullname NOT LIKE '$search_user_text%' and status='Active' and (select count(*) from foot_blocked_user where (other_user_id='".$user_id."' and foot_blocked_user.user_id=foot_app_users.user_id and block_status='active')  or (other_user_id=foot_app_users.user_id and foot_blocked_user.user_id='".$user_id."' and block_status='active')  )  ='0'  order by fullname ASC) limit $start,$limit");     
  // $query = $this->db->query("(SELECT fullname,profilecover,profileimage,total_post,status,user_id,username FROM foot_app_users where user_type='user' and substring(fullname, 1, 6) between '$search_user_text' and 'z%' and status='Active' and (select count(*) from foot_blocked_user where (other_user_id='".$user_id."' and foot_blocked_user.user_id=foot_app_users.user_id and block_status='active') or (other_user_id=foot_app_users.user_id and foot_blocked_user.user_id='".$user_id."' and block_status='active') ) ='0' ORDER BY `foot_app_users`.`fullname` ASC ) limit $start,$limit");
    $query = $this->db->query("(SELECT fullname,profilecover,profileimage,total_post,status,user_id,username FROM foot_app_users where user_type='user' and fullname LIKE '$search_user_text%' and status='Active' and (select count(block_id) from foot_blocked_user where (other_user_id='".$user_id."' and foot_blocked_user.user_id=foot_app_users.user_id and block_status='active') or (other_user_id=foot_app_users.user_id and foot_blocked_user.user_id='".$user_id."' and block_status='active') ) ='0' ORDER BY `foot_app_users`.`fullname` ASC ) limit $start,$limit");
...
public function search_user_list_v2($data)
  {
        $search_user_text = $data->search_text;
        ...
        $limit = 20;
     // $q= "SELECT * FROM foot_app_users where user_type='user' and fullname LIKE '$search_user_text%' and status='Active' limit $start,$limit";
 echo $q="SELECT foot_app_users.* ,count(ff1.follow_id) as following ,count(ff2.follow_id) as followers, 
 IF(ff1.from_id = ".$user_id.", '1', '0') is_following,
 IF(ff2.to_id = ".$user_id.", '1', '0') is_follower  FROM foot_app_users 
left join foot_blocked_user fbu on fbu.user_id = foot_app_users.user_id 
left join foot_blocked_user fbu2 on fbu2.other_user_id = foot_app_users.user_id
left join foot_follow ff1 on ff1.from_id = foot_app_users.user_id
left join foot_follow ff2 on ff2.to_id = foot_app_users.user_id
where fbu.user_id != ".$user_id." and fbu2.other_user_id != ".$user_id." and foot_app_users.user_type='user' and foot_app_users.fullname 
LIKE '".$search_user_text."%' and foot_app_users.status='Active' 
and ff1.follow_status='active' and ff2.follow_status='active' group by foot_app_users.user_id order by foot_app_users.fullname limit $start,$limit"; die;
  ...
  function get_user_detail($comment_user_id,$user_id)
  {
...
// print_r($resultuser);die();
                ...
                // $resultuser['profile_url'] =base_url()."userprofile/index.php?user_id=".$comment_user_id;
...
public function check_following($data)
  {
    $user_id =  $data->user_id;   
   ...
   // $restultsfollow = array_shift($restults);
    ...
    if($restults)
    {
      ...
      $resultuser= $restults[0]['follow_status'];
    }
    else
    {
      $resultuser = "";
  ...
  {  
    ...
    { 
      ...
      {
          if($restultsfollow->follow_status == 'active')
          {
                  $dataUsers = array(
        ...
        $myUserDetail = $this->User_model->single_users_detail($user_id);
          ...
          $is_private = $NotificationUserDetail->is_private;
...
// print_r($resultuser);die();
  ...
  function user_like_personal_detail($comment_user_id,$user_id)
  {
   ...
   public function ajax_list_items($search='',$per_page=10,$start=0) 
    ...
    public function search_following_list($data)
  {
...
//         $this->db->join('foot_app_users',"foot_app_users.user_id = foot_follow.to_id and foot_app_users.fullname LIKE '$search_user_text'"); 
...
//         $query = $this->db->get();
        ...
        // $resultPosts = $query->result();
          ...
          // $query = $this->db->query("SELECT fullname,profilecover,profileimage,total_post,status,user_id,username FROM foot_app_users  where  user_id!='".$user_id."' and user_type='user' and username LIKE '$search_user_text%' and status='Active' and (select count(*) from foot_blocked_user where (other_user_id='".$user_id."' and foot_blocked_user.user_id=foot_app_users.user_id and block_status='active')  or (other_user_id=foot_app_users.user_id and foot_blocked_user.user_id='".$user_id."' and block_status='active')  )  ='0'  order by username asc  ");
          $query = $this->db->query("SELECT foot_app_users.*, foot_follow.* FROM foot_follow join foot_app_users on foot_app_users.user_id =foot_follow.to_id where foot_follow.follow_status= 'active' and foot_app_users.status='Active' and foot_follow.from_id='".$user_id."' and (foot_app_users.fullname LIKE '%$search_text%' or foot_app_users.username LIKE '%$search_text%') order by fullname asc");
    ...
    public function suggestion_list($data)
  {
        ...
        $limit = 50;
         ...
         $this->db->select('foot_follow.to_id');
       ...
       // $this->db->select('foot_app_users.username,foot_app_users.fullname,foot_app_users.useremail,foot_app_users.profileimage,foot_app_users.profilecover,foot_app_users.user_type,foot_app_users.total_points,foot_app_users.total_post,foot_app_users.is_private,foot_follow.*');

         // $this->db->from('foot_app_users');
          

          //$this->db->join('foot_follow',"foot_follow.from_id = foot_app_users.user_id"); 

          $where = "from_id='$user_id' and follow_status='active' ";
       
          $this->db->where($where);  
          // $this->db->where_not_in('productID', $already_used);
          //$this->db->order_by('foot_app_users.user_id', 'RANDOM');
          //$this->db->limit(3);
          $query = $this->db->get();
       $getfollowersArray = $query->result();
    if($getfollowersArray)
    {
    $arrayfollowers = array();
     foreach ($getfollowersArray as   $value) 
         { 
     $arrayfollowers[] = $value->to_id;
         }
    // return $arrayfollowers;
//     $this->db->select('foot_follow.to_id');

//          $this->db->from('foot_follow');
//     $this->db->where_in('from_id', $arrayfollowers);
...
//      $query = $this->db->get();
    ...
    //$myfollowers =implode(",",$arrayfollowers);
...
//           $this->db->order_by("foot_app_users.fullname", "asc");
       ...
       //return $resultPosts = $myfollowers;
        
        $result=array();
         $users = array();
//           $this->db->select('foot_app_users.user_id,foot_app_users.username,foot_app_users.fullname,foot_app_users.useremail,foot_app_users.profileimage,foot_app_users.profilecover,foot_app_users.user_type,foot_app_users.total_points,foot_app_users.total_post,foot_app_users.is_private,foot_follow.*');
...
//           $this->db->join('foot_follow',"foot_follow.from_id = foot_app_users.user_id"); 
         /* $this->db->select('foot_follow.to_id,foot_follow.from_id');
         ...
         $this->db->from('foot_follow');
          ...
          $this->db->where($where);
          $this->db->order_by('from_id', 'RANDOM');
          $this->db->limit(10);
          $query = $this->db->get();  */
  // $getresult =$query->num_rows();
       $qqq="SELECT foot_follow.to_id,foot_follow.from_id,'1' as is_followed FROM foot_follow where from_id in (".implode(',',$arrayfollowers).") and  to_id not in (".implode(',',$arrayfollowers).")  and follow_status='active' UNION ";

      $qqq=$qqq."SELECT foot_follow.to_id,foot_follow.from_id,'0' as is_followed FROM foot_follow where follow_status='active' order by is_followed desc limit $limit ";
...
//             $finalresult[$value->from_id][] =  $value;
//             $random_numbers= array_rand($finalresult[$value->from_id], 1);
                
                
// // 				$getarray[] = $finalresult[$value->from_id][$random_numbers]; 
			
// //             return $getarray;
//             // $getarray[] = $finalresult[$random_numbers];
//            // $getarray[]= array_rand($finalresult[$key], 1);
//             // if ($key > 3) break;
//           }
       ...
       // $orginalResult =array_slice($finalresult, $start, $limit);
